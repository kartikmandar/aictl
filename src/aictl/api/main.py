...
from aictl.clients.database import init_db, create_inbox_message
from aictl.services import session_service, terminal_service, flow_service, inbox_service
from aictl.services.cleanup_service import cleanup_old_data
from aictl.services.inbox_service import LogFileHandler
from aictl.services.terminal_service import OutputMode
from aictl.models.terminal import Terminal, TerminalId
from aictl.constants import SERVER_VERSION, SERVER_HOST, SERVER_PORT, TERMINAL_LOG_DIR, INBOX_POLLING_INTERVAL
from aictl.utils.logging import setup_logging
from aictl.utils.terminal import generate_session_name
from aictl.providers.manager import provider_manager
...
async def lifespan(app: FastAPI):
    ...
    logger.info("Starting AICTL server...")
    setup_logging()
    init_db()
    ...
    logger.info("Shutting down AICTL server...")


app = FastAPI(
    title="AICTL",
    description="Simplified AICTL API",
...
async def health_check():
    return {"status": "ok", "service": "aictl"}
...
async def list_terminals_in_session(session_name: str) -> List[Dict]:
    ...
    try:
        from aictl.clients.database import list_terminals_by_session
...
def main():
    """Entry point for aictl-server command."""
    import uvicorn
