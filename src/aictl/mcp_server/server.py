"""AICTL MCP Server implementation."""
...
from aictl.mcp_server.models import HandoffResult
from aictl.constants import DEFAULT_PROVIDER, API_BASE_URL
from aictl.models.terminal import TerminalStatus
from aictl.utils.terminal import generate_session_name, wait_until_terminal_status
...
mcp = FastMCP(
    ...
    This server provides tools to facilitate terminal delegation within AICTL sessions.

    ## Best Practices

    - Use specific agent profiles and providers
    - Provide clear and concise messages
    - Ensure you're running within an AICTL terminal (AICTL_TERMINAL_ID must be set)
    """
)
...
def _create_terminal(agent_profile: str) -> Tuple[str, str]:
    ...
    current_terminal_id = os.environ.get('AICTL_TERMINAL_ID')
...
def _send_to_inbox(receiver_id: str, message: str) -> Dict[str, Any]:
    ...
    Raises:
        ValueError: If AICTL_TERMINAL_ID not set
        Exception: If API call fails
    """
    sender_id = os.getenv("AICTL_TERMINAL_ID")
    if not sender_id:
        raise ValueError("AICTL_TERMINAL_ID not set - cannot determine sender")
...
async def handoff(
    ...
    """Hand off a task to another agent via AICTL terminal and wait for completion.
    ...
    - Must be called from within an AICTL terminal (AICTL_TERMINAL_ID environment variable)
...
async def assign(
    ...
    message: str = Field(
        description='The task message to send. Callback instructions are automatically added.'
    )
) -> Dict[str, Any]:
    """Assigns a task to another agent without blocking.

    This function automatically appends callback instructions to your message, so the worker agent
    knows how to send results back to you. You don't need to manually include callback instructions.

    The worker agent will automatically receive instructions to send results back to your terminal.

    Args:
        agent_profile: Agent profile for the worker terminal
        message: Task message (callback instructions are automatically added)

    Returns:
        Dict with success status, worker terminal_id, and message
    """
    try:
        # Get current terminal ID (supervisor)
        current_terminal_id = os.environ.get('AICTL_TERMINAL_ID')

        # Create terminal
        terminal_id, _ = _create_terminal(agent_profile)

        # Automatically append callback instructions if supervisor ID is known
        if current_terminal_id:
            enhanced_message = f"""{message}

---
**Callback Instructions**: When you complete this task, send your results back to the supervisor using:
send_message("{current_terminal_id}", "Your results here")

The supervisor's terminal ID is: {current_terminal_id}
"""
        else:
            enhanced_message = message

        # Send message immediately
        _send_direct_input(terminal_id, enhanced_message)
